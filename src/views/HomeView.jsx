...
import { ShoppingBag, ChevronRight, Sparkles, ShieldCheck, Truck, RefreshCw, PackageOpen } from 'lucide-react';

export default function HomeView() {
  const { productos, cargandoProductos, agregarAlCarrito, navegarA } = useStore();
  ...
  return (
    <div className="w-full space-y-16 py-6 px-6 sm:px-10 md:px-16 font-sans">
      ...
      {/* 1. HERO BANNER FULL-WIDTH ROSSELY */}
      <section className="w-full rounded-3xl overflow-hidden border border-[#F8D7E0]/80 bg-gradient-to-r from-[#FFFBFB] via-[#FDF5F7] to-[#FAF0F3] shadow-xs">
...
function ProductCardRossely({ producto, onSelect, onAdd }) {
  ...
  return (
    <div className="w-full bg-white border border-[#FCE4EC] rounded-3xl overflow-hidden hover:shadow-md transition duration-300 flex flex-col justify-between group">
      ...
      <div 
        ...
        onClick={onSelect}
      >
        <span className="absolute top-3 left-3 bg-[#701A3B] text-white text-[9px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full z-10">
          ROSSELY
        </span>
      ...
      <div className="p-5 flex flex-col flex-1 justify-between space-y-4">
        <div>
          ...
          <h4 onClick={onSelect} className="font-serif font-bold text-sm text-stone-900 hover:text-[#701A3B] cursor-pointer mt-1 line-clamp-1">
            {producto.nombre}
          </h4>
          
          <div className="flex items-baseline gap-2 mt-2">
            <p className="text-lg font-bold text-[#701A3B]">S/. {parseFloat(producto.precio).toFixed(2)}</p>
            {producto.precioOferta && (
              <p className="text-xs text-stone-400 line-through">S/. {parseFloat(producto.precioOferta).toFixed(2)}</p>
            )}
          </div>
        </div>
        ...
        {esAdmin ? (
          <button 
            ...
            className="w-full bg-[#FDF5F7] text-[#701A3B] border border-[#F8D7E0] font-semibold py-2.5 rounded-xl text-xs uppercase tracking-wider hover:bg-[#FCE4EC] transition cursor-pointer"
          >
            Modo Admin: Editar
          </button>
        ) : (
